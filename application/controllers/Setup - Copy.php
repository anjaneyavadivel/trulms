        ...
        if (isset($employeeRolw) && $employeeRolw->num_rows() > 0) {
            foreach ($employeeRolw->result() as $value) {
                ...
                if (selfAllowed($pagealterpermission, 'selfApprovalAllowed', $value->createby) && checkpageaccess('form-access', 1, 'approve')) {
                    $APPROVE = "<a href='" . base_url() . "approve-form-access-list/" . $value->pageRoleMappingID . "'role='button' tabindex='0' class='edit' data-toggle='tooltip' data-placement='top' title data-original-title='Click to View History'><i class='fa fa-clock-o'></i></a>&nbsp;";
                }
